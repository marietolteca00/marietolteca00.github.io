...
knitr::include_graphics(here::here("blog_post","eddy_co_blog_stats_222","image","DAG.png"))
```
...
\end{align}
$$ 
*Null (H0) : When there is no precipitation ETo stays the same.*

*Alt (HA): When there is precipitation, ETo decreases.*
...
*Null (H0): When Temperature is the same, Eto stays the same.*

*Alt (HA): When Temperature increase there is more ETo*
...
In our simulated data, we were able to recovered the true coefficients, let's begin by loading in our data from the Camarillo Station. I will be using two datasets that were retrieved from the CIMIS website, one was for daily ETo and daily data that consisted of precipitation (inches) and temperature (Fahrenheit). 
